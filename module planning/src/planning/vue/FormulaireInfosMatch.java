...
public abstract class FormulaireInfosMatch extends JFrame {
	...
	public FormulaireInfosMatch(Match match, List<Joueur> joueursMatch, Arbitre arbitre, List<Joueur> listJoueurs, List<Arbitre> listArbitres, Connection connection, PlanningOrga mainFrame) {
	...
	protected boolean verifierDonnees() {
		...
		if (!verifierNombreMatchArbitre()) {
			...
			System.out.println("Les arbitres ne doivent pas déjà être occupés sur un autre match");
			...
			System.out.println("Les deux joueurs doivent être différents");
		} else {
	...
	protected void persisterDonnees() {
		// TODO ajouter la persitance des données
		/*
		 * Mise à jour des données dans la bd
		 * 
		 * - update le match avec jour/heure/court V
		 * - enlever les joueurs qui jouaient le match V
		 * - ajouter les nouveaux joueurs V
		 * - enlever l'arbitre qui jugeait le match V
		 * - ajouter le nouvel arbitre V
		 * - attribuer automatiquement les arbitres de lignes et equipes de ramasseurs de balless
		 * 
		 */
